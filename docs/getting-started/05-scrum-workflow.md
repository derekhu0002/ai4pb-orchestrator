...
在项目导出完成 `design/KG/SystemArchitecture.json` 后，请优先使用 AI4PB 侧边栏中的当前流程按钮，而不是旧式的“手工打开某个 Prompt”操作。

## 步骤 1: Task List (任务清单排期)
- **触发**：在侧边栏点击 `计划与汇报流程 -> 待办梳理`。
...
- 若需要直接生成单任务执行说明，可在侧边栏点击 `执行支持流程 -> 执行支持`：
...
- **行动**：人工检查当前代码与任务说明、核对关键功能点并记录验收结论；若发现问题，先补充为 Issue 并更新 EA 导出，再进入缺陷修改流程。若需要在工作区中记录问题回填，可点击 `SCRUM敏捷开发流程 -> 问题反馈` 打开专用反馈编辑页。
- 一次性验收成功：
...
- **触发**：在侧边栏点击 `计划与汇报流程 -> 周报输出`。
![alt text](docs/getting-started/image-38.png)
- **目的**：为干系人生成易于理解的管理层周报 (`implementation/reports/xxx.md`)。涵盖执行摘要和表格级别的任务进展。

## 附：OpenCode 执行模式

如果某个阶段被路由到 OpenCode，AI4PB 会沿用同样的侧边栏按钮，不需要切换到另一套 UI。当前差异只体现在执行后端：

- `transport = "cli"` 时，扩展直接调用本地或 WSL 中的 `opencode` 命令
- `transport = "server"` 时，扩展通过 `opencode serve` 的 HTTP 接口创建会话、订阅事件并展示流式结果

也就是说，工作流入口不变，变化的是按钮背后的执行代理与传输方式。
