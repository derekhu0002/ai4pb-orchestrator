...
type OpenCodeTransport = 'cli' | 'server';

type OpenCodeExecutionHost = 'native' | 'wsl' | 'auto';
...
type WorkspaceProjectConfigCopySpec = {
...
type WorkflowFlowKey = 'delivery' | 'audit' | 'planning' | 'support';

type WorkflowMenuKey = 'auto' | 'flow' | 'config';
...
const WORKFLOW_FLOW_OPTIONS: Array<{
  key: WorkflowFlowKey;
  label: string;
  description: string;
  steps: Array<{ key: SkillKey; label: string }>;
}> = [
  {
    key: 'delivery',
    label: 'SCRUM敏捷开发流程',
...
const HELP_URLS = {
  ...
  } as Record<WorkflowFlowKey, string>,
  step: {
...
type WorkflowStreamBubbleState = {
  label: string;
  thinking: string;
  response: string;
  errorText: string;
...
type WorkflowThreadEntry =
  ...
  | {
      kind: 'streamBubble';
      streamId?: string;
      stream: WorkflowStreamBubbleState;
    }
  | {
      kind: 'autoSuggestion';
      text: string;
  ...
  | {
      kind: 'extractedTasks';
      tasks: Array<{id: string; name: string; checked: boolean; problem?: string; problemNotes?: string; resolverNotes?: string; problemType?: string; status?: string; objectId?: string; reporter?: string; assignedTo?: string; startDate?: string; dueDate?: string; priority?: string}>;
    };

type WorkflowViewState = {
  activeMenu: WorkflowMenuKey;
  confirmedMenu: 'auto' | 'flow' | null;
  menuOpen: boolean;
  expandedConfig: boolean;
  expandedFlow: WorkflowFlowKey | null;
  selectedFlow: WorkflowFlowKey | null;
  selectedSkill: SkillKey | null;
  draftText: string;
  thread: WorkflowThreadEntry[];
};

type IssueFeedbackRow = {
  lineIndex: number;
  objectId: string;
  name: string;
  problem: string;
...
type ParsedTaskIssuesTable = {
  filePath: string;
  lines: string[];
  headerCells: string[];
  resolverNotesColumn: number;
  statusColumn: number;
  objectIdColumn: number;
  nameColumn: number;
  problemColumn: number;
  rows: Array<{
    lineIndex: number;
    cells: string[];
  }>;
...
class WorkflowViewProvider implements vscode.WebviewViewProvider {
  ...
  private readonly runtimeStreams = new Map<string, WorkflowStreamBubbleState>();
  ...
  public openInEditor(): void {
    ...
    panel.webview.onDidReceiveMessage(async (message: { command?: string; type?: string; key?: string; skill?: string; text?: string; url?: string; state?: WorkflowViewState; tasks?: any[] }) => {
  ...
  public resolveWebviewView(webviewView: vscode.WebviewView): void {
    ...
    webviewView.webview.onDidReceiveMessage(async (message: { command?: string; type?: string; key?: string; skill?: string; text?: string; url?: string; state?: WorkflowViewState; tasks?: any[] }) => {
      if (message.type === 'syncState' && message.state) {
  ...
  private async handleChatRequest(rawText?: string, rawSkill?: string): Promise<void> {
    ...
    if (explicitSkill === 'extract-tasks') {
      try {
        ...
        const tasks: Array<{ id: string; name: string; checked: boolean; problem?: string; problemNotes?: string; resolverNotes?: string; problemType?: string; status?: string; objectId?: string; reporter?: string; assignedTo?: string; startDate?: string; dueDate?: string; priority?: string }> = [];
  ...
  private async handleSubmitExtractedTasks(tasks: Array<{id: string; name: string; checked: boolean; problem?: string; problemNotes?: string; resolverNotes?: string; problemType?: string; status?: string; objectId?: string; reporter?: string; assignedTo?: string; startDate?: string; dueDate?: string; priority?: string}>): Promise<void> {
    try {
      const root = getWorkspaceRoot();
  ...
  private async openIssueFeedbackEditor(): Promise<void> {
    const root = getWorkspaceRoot();
    ...
    const activeRows = parsed.rows
      ...
      .filter((row): row is IssueFeedbackRow => Boolean(row));
    ...
    panel.webview.onDidReceiveMessage(async (message: { type?: string; feedbacks?: Array<{ lineIndex?: number; feedback?: string }> }) => {
...
<body>
  ...
  <script nonce="${nonce}">
    ...
    function normalizeStreamContent(content, fallbackStatus) {
      ...
      response = response.replace(/<think>([\s\S]*?)(?:<\/think>|$)/g, (match, p1) => {
  ...
  const menuOpen = raw.menuOpen === true;
    ...
    for (const item of rawThread) {
      ...
      if (item.kind === 'streamBubble') {
        ...
        thread.push({
          kind: 'streamBubble',
          streamId: typeof item.streamId === 'string' ? item.streamId : undefined,
          stream: {
...
type EffectiveOpenCodeServerConfig = {
  baseUrl: string;
  username: string;
  password: string;
  sessionTitle: string;
...
type EffectiveOpenCodeConfig = Required<Omit<OpenCodeCliConfig, 'transport' | 'server'>> & {
  transport: OpenCodeTransport;
  server: EffectiveOpenCodeServerConfig;
};
...
type OpenCodeInvocation = {
  ...
  timeoutMs: number;
  executionHost: OpenCodeExecutionHost;
  wslDistribution?: string;
  wslUseLoginShell: boolean;
  serverBaseUrl: string;
  serverUsername: string;
  serverPassword: string;
  serverSessionTitle: string;
...
type OpenCodeStreamHandlers = {
  ...
  onResponseDelta?: (chunk: string) => void;
...
type OpenCodeServerPromptResponse = {
  info?: {
    id?: string;
    error?: {
      name?: string;
...
async function analyzeAutoSkillSuggestions(userText: string): Promise<AutoSkillSuggestion[]> {
  ...
  try {
    ...
    const normalized = parsed
      ...
      .filter((item): item is AutoSkillSuggestion => Boolean(item));
...
function safeParseJson(raw: string): ParsedAutoSuggestion[] | undefined {
  try {
    const parsed = JSON.parse(raw) as ParsedAutoSuggestion | ParsedAutoSuggestion[];
...
// @ArchitectureID: 1209
function buildFallbackAutoSkillSuggestions(userText: string): AutoSkillSuggestion[] {
...
// @ArchitectureID: 1209
function rankSkillsFromText(input: string): SkillKey[] {
  const text = input.toLowerCase();
  const scores = new Map<SkillKey, number>([
    ['init', 0],
    ['audit', 0],
    ['wrapup', 0],
    ['iteration-summary', 0],
  ...
  const addScore = (skill: SkillKey, value: number): void => {
...
// @ArchitectureID: 1209
function dedupeAutoSkillSuggestions(suggestions: AutoSkillSuggestion[]): AutoSkillSuggestion[] {
  const seen = new Set<SkillKey>();
  const deduped: AutoSkillSuggestion[] = [];
...
function normalizeOpenCodeExecutionHost(value?: string): OpenCodeExecutionHost | undefined {
...
function normalizeOpenCodeTransport(value?: string): OpenCodeTransport | undefined {
  switch ((value ?? '').trim().toLowerCase()) {
    case 'cli':
      return 'cli';
    case 'server':
...
function isCommandNotFoundFailure(errorText: string, exitCode: number): boolean {
  if (exitCode === 127) {
    return true;
  }
...
function stripKnownWslWarnings(text: string): string {
  if (!text) {
    return text;
  }
...
function normalizeOpenCodeBaseUrl(value?: string): string {
  const trimmed = (value ?? '').trim();
  const fallback = 'http://127.0.0.1:4096';
  return (trimmed || fallback).replace(/\/+$/, '');
}

function buildOpenCodeServerUrl(baseUrl: string, routePath: string, query?: Record<string, string>): string {
...
function buildOpenCodeServerAuthHeader(username: string, password: string): string | undefined {
  if (!password) {
    return undefined;
  }
...
function summarizeHttpErrorBody(bodyText: string): string {
...
async function requestOpenCodeServerJson<T>(
  method: string,
  urlString: string,
  headers: Record<string, string>,
  body: unknown,
...
function getAgentDisplayLabel(agent: AgentExecutor): string {
  return agent === 'opencode' ? 'OpenCode' : 'Copilot';
}

function buildSkillAgentDisplayMap(config?: AiConfig): Record<SkillKey, string> {
  return (Object.keys(SKILL_PROMPT_REFERENCE) as SkillKey[]).reduce<Record<SkillKey, string>>((accumulator, skill) => {
    accumulator[skill] = getAgentDisplayLabel(resolveAgentExecutorForSkill(skill, config));
    return accumulator;
  }, {} as Record<SkillKey, string>);
}

function getSkillAgentDisplayMapForWorkspace(): Record<SkillKey, string> {
  try {
    const root = getWorkspaceRoot();
...
function buildOpenCodeInvocation(context: OpenCodeExecutionContext, config?: AiConfig): OpenCodeInvocation {
  ...
  const configuredEnv = Object.entries(opencodeConfig.env).reduce<Record<string, string>>((accumulator, [key, value]) => {
    accumulator[key] = replaceOpenCodeTemplate(value, context);
    return accumulator;
  }, {});
  const env = Object.entries(configuredEnv).reduce<NodeJS.ProcessEnv>((accumulator, [key, value]) => {
...
async function runOpenCodeServerInvocation(
  invocation: OpenCodeInvocation,
  streamHandlers?: OpenCodeStreamHandlers
  ...
  const health = await requestOpenCodeServerJson<OpenCodeServerHealth>(
    'GET',
    buildOpenCodeServerUrl(invocation.serverBaseUrl, '/global/health'),
    headers,
    undefined,
...
function buildWslOpenCodeSpawn(invocation: OpenCodeInvocation): { command: string; args: string[]; env: NodeJS.ProcessEnv } {
  ...
  const envSegments = Object.entries(invocation.configuredEnv).map(
    ([key, value]) => `${key}=${quotePosixShellArg(value)}`
  );
...
async function runOpenCodeInvocation(invocation: OpenCodeInvocation, streamHandlers?: OpenCodeStreamHandlers): Promise<OpenCodeCommandResult> {
  if (invocation.transport === 'server') {
    return runOpenCodeServerInvocation(invocation, streamHandlers);
  }

  const shouldTryWslFirst =
    invocation.executionHost === 'wsl' ||
  ...
  if (shouldTryWslFirst) {
    ...
    const wslResult = await executeOpenCodeSpawn(
      wslSpawn.command,
      wslSpawn.args,
      undefined,
      wslSpawn.env,
      false,
      invocation.promptPayload,
      invocation.timeoutMs,
      streamHandlers
    );
  ...
  return executeOpenCodeSpawn(
    invocation.command,
    invocation.args,
    invocation.cwd,
    invocation.env,
    invocation.promptToStdin,
    invocation.promptPayload,
    invocation.timeoutMs,
    streamHandlers
  );
...
function parseTaskIssuesMarkdown(filePath: string): ParsedTaskIssuesTable | undefined {
...
function normalizeTaskTableCellForWrite(value: string): string {
  return String(value ?? '')
    .replace(/\r\n?|\n/g, '<br>')
    .replace(/\|/g, '\\|');
}
...
function applyIssueFeedbackToTaskTable(
  parsed: ParsedTaskIssuesTable,
  feedbacks: Array<{ lineIndex?: number; feedback?: string }>
): number {
  if (!Array.isArray(feedbacks) || feedbacks.length === 0) {
    return 0;
  }

  const rowMap = new Map<number, { lineIndex: number; cells: string[] }>();
...
function buildIssueFeedbackWebviewHtml(webview: vscode.Webview, rows: IssueFeedbackRow[]): string {
  const nonce = randomBytes(16).toString('base64');
...
function readBundledSkillText(skillRelativePath: string): string {
...
async function initializeAICodingAgentWorkspaceProjectConfig(): Promise<void> {
...
async function initializeWorkspaceProjectConfigs(specs: WorkspaceProjectConfigCopySpec[], label: string): Promise<void> {
  const root = getWorkspaceRoot();
  ...
  if (existingSpecs.length > 0) {
    ...
    const choice = await vscode.window.showWarningMessage(
      `初始化${label}配置将复制: ${allTargets}。已存在: ${existingTargets}`,
      { modal: true },
      '覆盖',
      '打开已有目录'
    );
  ...
  const revealChoice = await vscode.window.showInformationMessage(
    `已完成初始化${label}配置: ${initializedTargets}`,
    '在资源管理器中查看'
  );
...
function replaceDirectoryFromSource(sourceDir: string, targetDir: string): void {
  if (exists(targetDir)) {
...
// @ArchitectureID: 1220
function copyFilesRecursivelyOverwrite(sourceDir: string, targetDir: string): void {
