...
# PHASE 4: ARTIFACT & CONTEXT UPDATES (MANDATORY)

After you have verified the implementation for a task, you MUST perform two crucial document updates to preserve context and ensure traceability:

## 4.1 UPDATE TASK SUPPORT DOC WITH MANUAL TESTING STEPS
If you read a support document for this task from `implementation\taskhelpinfos\`:
1. Open the file you read earlier.
2. Locate the section `## 6. 交付物与验收标准` (or similar section defining acceptance criteria).
3. Underneath **each** existing acceptance criteria point in that section, you MUST insert a new block titled `***人工验收测试步骤 (Generated by Copilot):***` followed by concrete, actionable steps a human can take to verify the code you just wrote.
4. Ensure these manual testing steps are specific to the files you modified, commands to run, or UI clicks to perform based on your actual implementation details.

## 4.2 APPEND RESOLVER NOTES TO TASK INDEX
