...
2. 在 bootstrap 里设置：
  ...
  - bootstrap 会先从 VS Code 已安装扩展目录（`.vscode/extensions/*ai4pb-orchestrator*/...`）查找共享脚本，并按版本号降序选择最新版本
  - 若扩展目录下未找到，再回退到项目内候选路径（如 `<projectPath>/script/EA-jsscript/project_auto_gen_suitable_for_LLM-V2.js`）以及本机 `ai4pb-orchestrator` 仓库路径
