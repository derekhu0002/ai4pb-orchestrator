...
exports.scanTypeScriptReality = scanTypeScriptReality;
...
// @ArchitectureID: 1276
function scanTypeScriptReality(workspaceRoot, explicitExcludes, logger) {
    ...
    for (const sourceFile of project.getSourceFiles()) {
        ...
        for (const classDeclaration of sourceFile.getClasses()) {
            pushDeclarationElement(elements, relationships, includedElementIds, includedRelationshipIds, fileElement, relativePath, `ts:class:${relativePath}:${classDeclaration.getName() ?? 'anonymous'}`, classDeclaration.getName() ?? 'anonymous', 'Class', classDeclaration.isExported(), classDeclaration.hasModifier('export') ? 'public' : 'unknown');
        }
        for (const functionDeclaration of sourceFile.getFunctions()) {
            pushDeclarationElement(elements, relationships, includedElementIds, includedRelationshipIds, fileElement, relativePath, `ts:function:${relativePath}:${functionDeclaration.getName() ?? 'anonymous'}`, functionDeclaration.getName() ?? 'anonymous', 'Function', functionDeclaration.isExported(), 'public');
        }
        for (const interfaceDeclaration of sourceFile.getInterfaces()) {
            pushDeclarationElement(elements, relationships, includedElementIds, includedRelationshipIds, fileElement, relativePath, `ts:interface:${relativePath}:${interfaceDeclaration.getName()}`, interfaceDeclaration.getName(), 'Interface', interfaceDeclaration.isExported(), 'public');
        }
        for (const enumDeclaration of sourceFile.getEnums()) {
            pushDeclarationElement(elements, relationships, includedElementIds, includedRelationshipIds, fileElement, relativePath, `ts:enum:${relativePath}:${enumDeclaration.getName()}`, enumDeclaration.getName(), 'Enum', enumDeclaration.isExported(), 'public');
        }
        for (const typeAliasDeclaration of sourceFile.getTypeAliases()) {
            pushDeclarationElement(elements, relationships, includedElementIds, includedRelationshipIds, fileElement, relativePath, `ts:type:${relativePath}:${typeAliasDeclaration.getName()}`, typeAliasDeclaration.getName(), 'TypeAlias', typeAliasDeclaration.isExported(), 'public');
        }
...
// @ArchitectureID: 1276
function pushDeclarationElement(elements, relationships, includedElementIds, includedRelationshipIds, parentFile, relativePath, id, name, type, exported, visibility) {
    const element = {
        id,
        name,
        type,
...
//# sourceMappingURL=scanTypeScriptReality.js.map