...
当 `ProjectOrchestrator` 因 QA 或 Audit 连续失败而打开熔断器时，不再向人类发送开放式追问，而是发送一个单选恢复指令。`Human_Developer` 只需要从以下三项里选择一项：

- `[已修复代码]`：我（人类）已经手动改了代码并 commit，请直接重试 QA/Audit。系统会解除受影响任务的 `blocked` 状态，必要时先把最新人工 commit 回填到运行时任务，再直接重跑当前 lane 需要的验证。
- `[架构需重构]`：我决定变更架构，请把任务发回给 `SystemArchitect`，重置 `retryCount`。系统会把失败上下文带回架构环节，并把受影响任务的自动重试计数清零。
- `[忽略偏差]`：这是合理的架构偏差，请强行置为 Done。这个选项只用于架构/审计偏差，不用于忽略 QA 失败；系统会记录人工豁免并在其他闸门满足时继续流程。

这让人类的手工修复、架构重判和偏差豁免都能被编排器明确识别，并重新同步回自动循环，而不是停留在聊天窗口里。
