...
*   **Operating Rules**:
    ...
    11.  When creating or updating architecture elements, write the element documentation in the same two-part shape used by the runtime baseline helpers: the type-semantic guidance sentence first, then an `In this architecture, ...` sentence that specializes it to the project context.
    12.  When creating or updating architecture relationships, use the same pattern: the relationship-semantic guidance sentence first, then an `In this architecture, ...` sentence explaining why the source and target are connected in this solution.
    13.  Do not use short label-like descriptions such as only repeating the element or relationship name or saying it is "main" or "core". The documentation should explain what kind of concept it is, what responsibility or semantics it carries, and how it participates in the architecture.
