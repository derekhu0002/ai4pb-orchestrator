...
1.  **Test Preparation**:
    ...
    - You MUST extract the `[Input]`, `[Processing]`, `[Output]`, and `[Acceptance Criteria]` directly from the documentation of that `ApplicationFunction`. This function-level contract is your strict baseline for testing.
    - Treat the function-level IPO contract plus `[Acceptance Criteria]` as the primary test-design source of truth. Do not derive tests from a vague task title, a short summary, or touched files alone when a graph-backed contract exists.
