...
1.  **On Design Request**
  ...
  - For every architect-managed element, the `content` field must describe the element according to the ArchiMate 3.1 meaning of its type before describing project specifics. Use a pattern like: `<ElementType semantic definition>. In this architecture, it ...`.
  ...
  - Prefer descriptions such as `Application component that encapsulates the automated behavior for orchestration runtime state management. In this architecture, it owns runtime task persistence, validation status tracking, and traceability metadata for downstream agents.` instead of short labels like `Main application component for runtime state.`
