...
- [ ] 提供一个应用层内部服务入口，可在无显式 UI 触发的情况下启动现实扫描与协调流程。
...
- [ ] `reality.json` 的核心结构至少覆盖 `elements`、`relationships`、`views` 三类字段。
...
- [ ] 无法稳定映射的语言细节不会污染核心 schema，而是降级为扩展属性或 `未知`。
...
- [ ] TypeScript 扫描适配器实际基于 `ts-morph` 或同等级 AST 方案实现。
...
- [ ] Python 扫描适配器通过独立 Python 子进程执行，并遵守 stdin/argv 输入、stdout JSON 输出、非 0 退出码失败协议。
...
- [ ] 扫描器支持显式排除列表输入，且默认排除策略仅保留在扫描器内部。
...
- [ ] Architecture Reconciliation Service 同时读取 intent 模型和 `reality.json`，输出可消费的差异结果。
...
- [ ] `design/intention_reality_audit/` 下生成 JSON 与 Markdown 两种 ARCH_IMPL_GAP 报告。
...
- [ ] 输出覆盖率、准确率、纯净度原始指标；如输出总分，不能依赖硬编码阈值判定。
...
- [ ] 整个实现过程中不会把运行时扫描结果回写到 `design/KG/SystemArchitecture.json`。
